...
from ai.question_generator import generate_aptitude_questions
from database.interview_queries import save_aptitude_result
...
def conduct_test(user):
    ...
    st.subheader(f"{st.session_state.aptitude_category}")
    ...
    for idx, question in enumerate(questions):
        ...
        # safe store only if selected
        if selected:
            st.session_state.aptitude_answers[idx] = selected
...
def show_results(user):
    ...
    questions = st.session_state.aptitude_questions
    answers = st.session_state.aptitude_answers
    ...
    for idx, question in enumerate(questions):
        ...
        selected_letter = user_answer[:1] if user_answer else ""
    ...
    for i, result in enumerate(detailed_results):
        ...
        with st.expander(f"Question {i + 1}"):
            ...
            if result["status"]:
                st.success(f"Correct: {result['selected']}")
            else:
                st.error(f"Your Answer: {result['selected']}")
                st.info(f"Correct Answer: {result['correct']}")

            st.write(result["explanation"])

    save_results(user, correct, total, percentage)


# -----------------------------
# SAVE RESULTS
# -----------------------------
def save_results(user, correct, total, percentage):

    duration = datetime.now() - st.session_state.aptitude_start_time

    data = {
        "category": st.session_state.aptitude_category,
        "score": percentage,
        "total": total,
        "correct": correct,
        "time_taken": int(duration.total_seconds())
    }

    if "aptitude_saved" not in st.session_state:

        success = save_aptitude_result(user["id"], data)
